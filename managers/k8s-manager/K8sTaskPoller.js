...
class K8sTaskPoller {
    ...
    function poller(object, intervalId) {
      ...
      return eventmesh.apiServerClient.getResource({
              ...
              return eventmesh.apiServerClient.updateResource({
                ...
                .then(() => {
                  if (_.includes([CONST.APISERVER.RESOURCE_STATE.SUCCEEDED, CONST.APISERVER.RESOURCE_STATE.FAILED], resourceBody.status.state)) {
                    K8sTaskPoller.clearPoller(metadata.name, intervalId);
                  } else {
                    logger.info('Hello');
                    //Put your code here
        ...
        .catch(err => {
          ...
          K8sTaskPoller.clearPoller(object.metadata.name, intervalId);
        });
    }

    function startPoller(event) {
      logger.debug('Received Director Event: ', event);
      if ((event.type === CONST.API_SERVER.WATCH_EVENT.ADDED || event.type === CONST.API_SERVER.WATCH_EVENT.MODIFIED) && !K8sTaskPoller.pollers[event.object.metadata.name]) {
        ...
        K8sTaskPoller.pollers[event.object.metadata.name] = intervalId;
      }
    }
    const queryString = `state in (${CONST.APISERVER.RESOURCE_STATE.IN_PROGRESS})`;
    return eventmesh.apiServerClient.registerWatcher(CONST.APISERVER.RESOURCE_GROUPS.DEPLOYMENT, 'kubes', startPoller, queryString)
      .then(stream => {
  ...
  static clearPoller(resourceId, intervalId) {
    ...
    _.unset(K8sTaskPoller.pollers, resourceId);
  }
}

K8sTaskPoller.pollers = [];
module.exports = K8sTaskPoller;