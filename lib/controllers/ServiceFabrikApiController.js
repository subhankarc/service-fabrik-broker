...
class ServiceFabrikApiController extends BaseController {
  ...
  startBackup(req, res) {
    function getResourceAnnotationStatus(resourceType, resourceId, guid) {
      return Promise.try(() => {
        return resourceManager.manager.getAnnotationState(resourceType, resourceId, 'backup', 'default', guid);
      }).then(state => {
        if (state == CONST.RESOURCE_STATE.IN_QUEUE) {
          return getResourceAnnotationStatus(resourceType, resourceId, guid);
        } else {
          return resourceManager.manager.getAnnotationKey(resourceType, resourceId, 'backup', 'default', guid, 'result');
        }
      });
    }
    ...
    return Promise
      ...
      .spread((guid, deployment) => {
        ...
        //let deploymentName;
        return resourceManager.manager.annotateResource(req.manager.name, req.params.instance_id, 'backup', 'default', backup_guid, JSON.stringify(value));
        // return req.manager
        //   .startBackup(value);
      })
      .then(() => {
      ...
      .tap(response => {
        ...
        directorManager
          .findNetworkSegmentIndex(req.params.instance_id)
          .then(networkIndex => {
            logger.error('networkIndex is ', networkIndex);
            directorManager.getDeploymentName(req.params.instance_id, networkIndex)
          })
  ...
  getLastBackup(req, res) {
    ...
    const tenant_guid = req.query.namespace || req.entity.space_guid;
    return req.manager
  ...
  listBackups(req, res) {
    ...
    options.tenant_guid = req.query.namespace;
    return this.listBackupFiles(options)
      .then(body => res
        .status(200)
        .send(body)
