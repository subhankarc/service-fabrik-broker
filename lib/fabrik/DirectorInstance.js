...
>>>>>>> Introduction of new state as deployed.
=======
          // case 'create':
          //   return this.createSecurityGroup(operation.space_guid);
>>>>>>> Enabling binding for BOSH, Service Registration ebnabled.
        case 'update':
  ...
  pollAndUpdateResourceState() {
    Promise.delay(2000).then(() => {
      return resourceManager.manager.getResourceKey(this.manager.name, this.guid, 'result')
    }).then(resultStr => {
      ...
      return resourceManager.manager.updateResourceKey(this.manager.name, this.guid, 'lastoperation', JSON.stringify(resultValue)).then(() =>
        resourceManager.manager.updateResourceState(this.manager.name, this.guid, lastOperation.state)).then(() => lastOperation);
    }).then(operation => {
  ...
  saveResult(error, response) {
    ...
    return resourceManager.manager.updateResourceKey(this.manager.name, this.guid, 'result', JSON.stringify(resultValue)).then(() =>
      resourceManager.manager.updateResourceState(this.manager.name, this.guid, resourceState));
  }
  ...
  bind(params) {
    return this
      ...
      .then(credentials => {
        ...
        return resourceManager.manager.updateAnnotationKey(this.manager.name, this.guid, 'bind', 'default', params.binding_id, 'result', JSON.stringify(resultValue))
      })
      .then(() => resourceManager.manager.updateAnnotationState(this.manager.name, this.guid, 'bind', 'default', params.binding_id, CONST.RESOURCE_STATE.SUCCEEDED));
  }
