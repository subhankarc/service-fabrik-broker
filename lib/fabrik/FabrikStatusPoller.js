...
class FabrikStatusPoller {
  ...
  static start(instanceInfoInput, operation, user) {
    logger.info(`instance info input`, instanceInfoInput);
    return Promise
      .try(() => {
          ...
          return Promise.try(() => {
              ...
              return Promise.try(() => {
                ...
                .finally(() => {
                  ...
                  if (instanceInfo.operationFinished) {
                    logger.info(`Backup complete for guid : ${instanceInfo.backup_guid} -  deployment : ${instanceInfo.deployment} `, operationResponse);
                    eventmesh.server.updateAnnotationKey('director', instanceInfo.instance_guid, 'backup', 'default', instanceInfo.backup_guid, 'result', JSON.stringify(operationResponse));
                    eventmesh.server.updateAnnotationState('director', instanceInfo.instance_guid, 'backup', 'default', instanceInfo.backup_guid, CONST.RESOURCE_STATE.SUCCEEDED);
                    clearInterval(timer);
...
pubsub.subscribe(CONST.TOPIC.APP_STARTUP, (eventName, eventInfo) => {
  ...
  if (eventInfo.type === 'external') {
    // FabrikStatusPoller.restart('backup'); #todo should route this thorough etcd
  }
});
